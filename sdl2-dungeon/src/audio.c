 ...
 * use: ffmpeg -i in.mp3 -acodec pcm_s16le -ac 2 -ar 48000 out.wav
 */
 ...
 * offer a specific feature If flag is set, SDL will change the format in the
 * actual audio file structure (as opposed to gDevice->want)
 *
 ...
 * AUDIO_CHANNELS being 2, allow actual 1) SDL_AUDIO_ALLOW_ANY_CHANGE
 * Allow all changes above
 */
 ...
 *
 */
static inline void playAudio(const char* filename,
                             Audio* audio,
                             uint8_t loop,
                             int volume);
...
void initAudio(void) {
    ...
    /* want.userdata = new; */
    if ((gDevice->device = SDL_OpenAudioDevice(NULL, 0, &(gDevice->want), NULL,
                                               SDL_AUDIO_ALLOW_CHANGES)) == 0) {
...
Audio* createAudio(const char* filename, uint8_t loop, int volume) {
    ...
    if (SDL_LoadWAV(filename, &(new->audio), &(new->bufferTrue),
                    &(new->lengthTrue)) == NULL) {
        fprintf(stderr,
                "[%s: %d]Warning: failed to open wave file: %s error: %s\n",
    ...
    while (audio != NULL) {
        if (audio->length > 0) {
            ...
            if (music && audio->loop == 1 && audio->fade == 0) {
                ...
                tempLength = ((uint32_t)len > audio->length) ? audio->length
                                                             : (uint32_t)len;
            }

            SDL_MixAudioFormat(stream, audio->buffer, AUDIO_FORMAT, tempLength,
                               audio->volume);
